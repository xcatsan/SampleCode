//
//  CoreDataConflict_AppDelegate.m
//  CoreDataConflict
//
...
#import "CoreDataConflict_AppDelegate.h"
#import "Book.h"

@implementation CoreDataConflict_AppDelegate

@synthesize window;

/**
    Returns the support directory for the application, used to store the Core Data
    store file.  This code uses a directory named "CoreDataConflict" for
...
- (NSString *)applicationSupportDirectory {
    ...
    return [basePath stringByAppendingPathComponent:@"CoreDataConflict"];
}
...
- (NSPersistentStoreCoordinator *) persistentStoreCoordinator {
    ...
    NSURL *url = [NSURL fileURLWithPath: [applicationSupportDirectory stringByAppendingPathComponent: @"storedata.db"]];
...
- (IBAction) saveAction:(id)sender {
    ...
    if (![self.moc1 commitEditing]) {
        NSLog(@"%@:%s unable to commit editing before saving", [self class], _cmd);
    }
    if (![self.moc2 commitEditing]) {
        NSLog(@"%@:%s unable to commit editing before saving", [self class], _cmd);
    }
    ...
    if (![self.moc1 save:&error]) {
		NSLog(@"moc1 error: %@", error);
		NSLog(@"UserInfo: %@", [error userInfo]);
    }
    if (![self.moc2 save:&error]) {
		NSLog(@"moc2 error: %@", error);
		NSLog(@"UserInfo: %@", [error userInfo]);
    }
	NSLog(@"saved.");
}
...
- (void)awakeFromNib
{
	...
	if (!count) {
		...
		book.title = @"Programming in Objective-C 2.0";
		book.author = @"Stephen G. Kochan";

		[self.managedObjectContext save:&error2];
		if (error2) {
			NSLog(@"error2: %@");
		} else {
			NSLog(@"Addes a sample record.");
		}
	}
	[request release];
